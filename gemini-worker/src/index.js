...
export default {
  async fetch(request, env, ctx) {
    ...
    const corsOrigin = "https://ritikbilala.github.io";
    ...
    // 2. Handle REST API for Dashboard
    if (url.pathname === "/api/logs" && request.method === "GET") {
      const authHeader = request.headers.get("Authorization");
      if (authHeader !== "Bearer " + env.ADMIN_SECRET) {
        return new Response(JSON.stringify({ error: "Unauthorized. Missing or invalid ADMIN_SECRET." }), { 
          status: 401, 
      ...
      try {
        ...
        const totalResult = await env.db.prepare("SELECT COUNT(*) as count FROM session_transcripts").first();
        const total = totalResult ? totalResult.count : 0;

        const results = await env.db.prepare(
          "SELECT session_id, transcripts, token_count, created_at FROM session_transcripts ORDER BY created_at DESC LIMIT ? OFFSET ?"
    ...
    function appendToLog(role, text) {
      if (sessionLogs.length > 0) {
        let lastLog = sessionLogs[sessionLogs.length - 1];
        if (lastLog.role === role) {
          lastLog.text += text;
          lastLog.timestamp = Date.now();
          return;
        }
      }
      sessionLogs.push({ role: role, text: text, timestamp: Date.now() });
    }

    async function saveSessionToDatabase(env) {
      if (isSaved || sessionLogs.length === 0) return;
      isSaved = true;
      try {
        console.log(`[Worker] Saving session ${sessionId} to D1... (Tokens: ${tokenCount})`);
        const jsonTranscript = JSON.stringify(sessionLogs);
        await env.db.prepare(
          "INSERT INTO session_transcripts (session_id, transcripts, token_count) VALUES (?1, ?2, ?3)"
        ).bind(sessionId, jsonTranscript, tokenCount).run();
        console.log(`[Worker] Session ${sessionId} saved successfully.`);
      } catch (error) {
        console.error("[Worker] D1 Insert Error:", error.message);
      }
    }
    ...
    geminiWs.addEventListener("message", async (event) => {
      try {
        ...
        if (!setupComplete) {
                    ...
                    turns: [
                      {
                        ...
                        parts: [
                          {
                            text: "Hey! Please introduce yourself as Natasha, Ritik Bilala's assitant to help recruiters, collaborators, and stakeholders visiting his portfolio. Greet the visitor warmly and ask their name and how you can help. Keep it to one short sentence.",
                          },
                        ],
                      },
                    ],
          ...
          // Setup complete: parse and intercept messages
          try {
            const parsed = JSON.parse(data);
            
            // Capture text responses from the model
            if (parsed.serverContent && parsed.serverContent.modelTurn) {
              const parts = parsed.serverContent.modelTurn.parts;
              if (parts) {
                for (const part of parts) {
                  if (part.text) {
                    // Gemini 2.0 experimental and audio models often output their "thoughts" 
                    // wrapped in **asterisks** before the actual spoken text. Clean it up:
                    let spokenText = part.text.replace(/\*\*[\s\S]*?\*\*\s*/g, "");
                    if (spokenText.trim().length > 0) {
                      appendToLog("model", spokenText);
                    }
                  }
                }
              }
            ...
            const usage = parsed.usageMetadata || (parsed.serverContent && parsed.serverContent.usageMetadata);
            if (usage && usage.totalTokenCount) {
               // We add them up assuming they are per-turn. If they are cumulative, 
               // keeping Math.max() is safer, but normally in Gemini live API it's per turn.
               tokenCount += usage.totalTokenCount;
            }
            
            // Capture user audio transcription if provided by the model
            const inputTrans = parsed.inputTranscription || 
                              (parsed.serverContent && (parsed.serverContent.inputAudioTranscription || parsed.serverContent.inputTranscription));
                              
            if (inputTrans && inputTrans.text) {
              appendToLog("user", inputTrans.text);
            }
          } catch (e) {
            // ignore JSON parse errors for audio blobs
          }
        }
    ...
    server.addEventListener("message", (event) => {
      ...
      try {
        ...
        // Intercept user text inputs (excluding our automated system prompt)
        if (parsed.clientContent && parsed.clientContent.turns) {
          const parts = parsed.clientContent.turns[0].parts;
          if (parts) {
            for (const part of parts) {
              if (part.text && !part.text.includes("introduce yourself as Natasha")) {
                appendToLog("user", part.text);
              }
            }
          }
        }
    ...
    geminiWs.addEventListener("close", (event) => {
      ...
      ctx.waitUntil(saveSessionToDatabase(env));
    });
